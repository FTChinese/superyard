...
func (env APNEnv) ListMessage(p gorest.Pagination) ([]apn.MessageTeaser, error) {
	...
	teasers := make([]apn.MessageTeaser, 0)

	for rows.Next() {
		var t apn.MessageTeaser
		err := rows.Scan(
			&t.ID,
			&t.PageID,
			&t.Action,
...
func (env APNEnv) TimeZoneDist() ([]apn.TimeZone, error) {
	query := `
...
func (env APNEnv) DeviceDist() ([]apn.Device, error) {
	query := `
...
func (env APNEnv) InvalidDist() ([]apn.InvalidDevice, error) {
	query := `
	...
	devices := make([]apn.InvalidDevice, 0)

	for rows.Next() {
		var d apn.InvalidDevice
...
func (env APNEnv) CreateTestDevice(d apn.TestDevice) error {
	query := `
...
func (env APNEnv) ListTestDevice() ([]apn.TestDevice, error) {
	query := `
	SELECT id,
